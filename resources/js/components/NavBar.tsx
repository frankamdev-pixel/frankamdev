...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: 'Tableau de bord', href: '/dashboard', icon: <LayoutDashboard size={16} /> },
  ];
  ...
  return (
    <nav
      className={`fixed top-0 left-0 z-50 w-full transition-all ${
        scrolled ? 'border-b border-gray-700/50 bg-black/30 backdrop-blur-md' : 'bg-transparent'
      }`}
    >
          ...
          <div className="hidden md:flex space-x-8 font-medium text-white">
            {navLinks.map((link) =>
              link.name !== 'Dashboard' || auth.user ? (
                <Link key={link.name} href={link.href} className="transition hover:text-blue-400">
                  {link.name}
                </Link>
              ) : null
            )}
          ...
          <div className="relative ml-4" ref={dropdownRef}>
            {auth.user ? (
              <div
                ...
                className="group relative flex items-center gap-2 cursor-pointer text-white"
              >
                {auth.user.profile_photo_url ? (
                  <div className="relative">
                    ...
                    />
                    <Pointer className="absolute top-12 -right-6 -translate-y-1/2 animate-bounce text-yellow-400" size={20} />
                  </div>
                ) : (
              ...
              >
                <div className="ring-opacity-5 absolute right-0 z-50 mt-2 w-64 rounded-lg bg-white py-2 shadow-lg ring-1 ring-black">
                  ...
                  <div className="rounded-t-lg bg-blue-50 px-4 py-3 text-sm font-semibold text-gray-800">
                    👋 Salut, <span className="text-blue-600">{auth.user.name}</span>
                  </div>
